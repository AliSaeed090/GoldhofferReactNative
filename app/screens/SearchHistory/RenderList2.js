...
export default function RenderList2({ item }) {
    ...
    return (
        <TouchableOpacity  disabled={item.disabled}  onPress={() => navigation.navigate(item.navigateTo, { item: item.data })} style={{ width: '100%', backgroundColor: '#D2D9DE', height: 200 }}>
            <View style={{ height: 200, justifyContent: 'center', alignItems: 'center' }}>
                <Image source={item.image} style={styles.banner} resizeMode="cover" />

            </View>
            <TouchableOpacity disabled={item.disabled}  onPress={() => navigation.navigate(item.navigateTo, { item: item.data })}
